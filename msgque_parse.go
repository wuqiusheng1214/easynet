...
@Time       : 2022/1/3
@Author     : wuqiusheng
@File       : msgque_parse.go
@Description: 消息解析器，解析器，解析器工厂
*/
package easynet
...
func (r *Parser) GetRemindMsg(err error, t MsgType) *Message {
	if t != MsgTypeCmd {
		return nil
	}
...
func (r *Parser) RegisterFunc(id uint16, c2sFunc ParseFunc, s2cFunc ParseFunc) {
	if r.msgMap == nil {
		r.msgMap = map[uint16]MsgParser{}
	}

	r.msgMap[id] = MsgParser{c2sFunc: c2sFunc, s2cFunc: s2cFunc}
}

func (r *Parser) Register(id uint16, c2s interface{}, s2c interface{}) {
	...
	r.RegisterFunc(id, c2sFunc, s2cFunc)
}
