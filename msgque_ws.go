...
func (r *wsMsgQue) readMsg() {
	for !r.IsStop() {
		...
		msgHead := &MessageHead{}
		msgHead.FromBytes(data)
		if !r.processMsg(r, &Message{Head: msgHead, Data: data[MsgHeadSize:]}) {
			break
		}
...
func (r *wsMsgQue) writeMsg() {
	r.writeCmd()
}

func (r *wsMsgQue) writeCmd() {
	var m *Message
	gm := MsgqueBroadcast.GetNewMsg()
	...
	for !r.IsStop() || m != nil {
		if m == nil {
			...
			case <-gm.C:
				...
				gm = MsgqueBroadcast.GetNextMsg(gm)
			case <-tick.C:
				if r.isTimeout(tick) {
					r.Stop()
				}
